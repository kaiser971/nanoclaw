...
async function main(): Promise<void> {
  ...
  startIpcWatcher({
    ...
    enqueueContainerRun: (chatJid, prompt, groupFolder) => {
      ...
      queue.enqueueTask(chatJid, `autoapply-pipeline-${Date.now()}`, async () => {
        ...
        if (totalToGenerate === 0) {
          logger.info('[AUTOAPPLY] No offers to generate CV for, stopping pipeline');
        ...
        if (cvTask) {
          ...
          logger.info({ result: cvResult.result }, '[AUTOAPPLY] CV generation check');

          if (cvResult.triggerContainer) {
            await sendMsg(`✏️ *Génération CV* : ${totalToGenerate} CV à adapter...`);
        ...
        if (pdfTask) {
          ...
          logger.info({ result: pdfResult.result }, '[AUTOAPPLY] PDF generation completed');
