 ...
 */
export function getUnscoredOffers(site?: string, profile?: string): ResolvedOffer[] {
 ...
 */
export function getScoredWithoutCV(site?: string, profile?: string): ResolvedOffer[] {
  const recuOffers = getOffersByStatus('RECU', site, profile);
  return recuOffers.filter((offer) => {
    ...
    return !files.some((f) => f.startsWith('CV_') && f.endsWith('.docx')) &&
 ...
 */
export function getPendingOffers(site?: string, profile?: string): ResolvedOffer[] {
  const recuOffers = getOffersByStatus('RECU', site, profile);
  return recuOffers.filter((offer) => {
    const hasScoringJson = fs.existsSync(path.join(offer.folderPath, 'SCORING.json'));
    if (!hasScoringJson) return true; // not scored yet
    // Scored — check if it's apply/maybe without CV
    try {
      const scoring = JSON.parse(fs.readFileSync(path.join(offer.folderPath, 'SCORING.json'), 'utf-8'));
...
export function getOfferByFolder(folderPath: string): ResolvedOffer | null {
  ...
  try {
    ...
    const status = (['RECU', 'APPLIED', 'ARCHIVED'].includes(statusDir)
      ? statusDir
      : 'RECU') as OfferLifecycle;
...
export function processScoringResults(): ScoringBilan {
  ...
  for (const offer of recuOffers) {
    ...
    if (rec === 'apply') {
      ...
      fs.writeFileSync(path.join(offer.folderPath, 'cause.md'), causeMd, 'utf-8');
...
export function archiveExpiredOffers(days: number): number {
  ...
  for (const offer of recuOffers) {
    ...
    try {
      ...
      const ageInDays = Math.round((Date.now() - collectedAt) / (1000 * 60 * 60 * 24));
